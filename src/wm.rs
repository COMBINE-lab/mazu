...
/// A "slice" of a wavelet matrix (WM), can be treated like a WM built for a slice of a sequence.
...
/// Implementation of the (faster) extended Wavelet Matrix.
///
/// From:
...
impl WaveletMatrix {
    ...
    /// New with rank and select support.
    ...
    /// Return height of wavelet matrix, i.e. `\ceil{ \lg \sigma }`
    ...
    pub fn access(&self, i: usize) -> Symbol {
        debug_assert!(self.supports_access());
        let mut j = i;
        let mut alpha = 0;
    ...
    pub fn rank(&self, a: Symbol, i: usize) -> usize {
        debug_assert!(self.supports_rank(), "Rank not supported");
        ...
        debug_assert!((a as usize) < self.alpha_size);
    ...
    pub fn select(&self, a: Symbol, rank: usize) -> Option<usize> {
        debug_assert!(self.supports_select(), "Select not supported");
