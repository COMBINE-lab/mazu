...
// Note: no need for to_pf1_info!
// impl SparseIndex {
//     pub fn to_pf1_info(&self) -> PF1Info {
//         let base = &self.base;
//         let metadata = base.metadata.as_ref().expect("pufferfish (c++ version) must have metadata");
...
//             have_edge_vec: metadata.have_edge_vec,
//             seq_hash: metadata.seq_hash.clone(),
//             name_hash: metadata.name_hash.clone(),
//             seq_hash_512: metadata.seq_hash_512.clone(),
//             name_hash_512: metadata.name_hash_512.clone(),
//             decoy_seq_hash: metadata.decoy_seq_hash.clone(),
//             decoy_name_hash: metadata.decoy_name_hash.clone(),
//             num_decoys: metadata.num_decoys,
//             first_decoy_index: metadata.first_decoy_index,
//             keep_duplicates: metadata.keep_duplicates,
...
mod test {
    ...
    fn pufferfish_type() {
        ...
        assert_eq!(pi.index_type(), ModIndexType::PF1Sparse);
        assert_eq!(pi.as_k2u().sample_size, 9);
        assert_eq!(pi.as_k2u().extension_size, 4);
    }
