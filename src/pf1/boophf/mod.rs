...
// - No functionality to build BooPHF
...
use hash::{MultiHash, MultiHashResult, MultiHashState};

use super::cpp::{read_u64_vec_with_len, DeserializeFromCpp, ReadFrom};
use crate::kphf::MPHF;
use crate::Result;
...
impl<T: Hash + Eq> BooPHF<T> {
    pub fn lookup(&self, item: &T) -> Option<u64> {
    ...
    pub fn num_bits(&self) -> usize {
        ...
        let nb =
            std::mem::size_of::<f64>() + std::mem::size_of::<u64>() + std::mem::size_of::<u64>();

        let levels_bits = {
            assert_eq!(self.levels.capacity(), self.levels.len());
            let mut nb = std::mem::size_of::<Vec<BoophfBitVec>>() * 8;
        ...
        let bytes_per_kv = std::mem::size_of::<T>() + std::mem::size_of::<u64>();
        let final_hash_nb =
            std::mem::size_of::<HashMap<T, u64>>() + self.final_hash.capacity() * bytes_per_kv;

        (nb * 8) + (final_hash_nb * 8) + levels_bits
    }
}

impl MPHF for BooPHF<u64> {
...
impl BoophfBitVec {
    ...
    pub fn num_bits(&self) -> usize {
        let ranks_bytes =
            std::mem::size_of::<Vec<usize>>() + self.ranks.len() * std::mem::size_of::<usize>();

        self.data.num_bits() + ranks_bytes * 8
    }
...
mod test {
    ...
    fn get_bbhash_small_example() -> BooPHF<u64> {
        ...
        let example_fp = "test_data/pf1/bbhash_n=10.bin";
